...
_CLAIM_REFUND = "app_secure.claim_pay10_refund_provider_execution(uuid,integer)"
_BIND_REQUEST = (
...
def _require_c_predecessor(bind) -> None:
    if bind.execute(
        sa.text(
            "SELECT pg_catalog.has_schema_privilege("
            ":role,'app_secure','USAGE')"
    ...
    if bind.execute(
        ...
        raise RuntimeError(
            "PAY-10-C refuses preexisting security-owner CREATE"
        )
    required_acl = bind.execute(
        sa.text(
            ...
            SELECT
                ...
                AND pg_catalog.has_table_privilege(
                    'app_security_owner','finance.refunds','SELECT,UPDATE'
                )
                AND pg_catalog.has_table_privilege(
                    'app_security_owner',
                    'finance.payment_allocations',
    ...
    for signature in _PAY10_C_FUNCTIONS:
        if bind.execute(
            sa.text(
...
def _install_c_capabilities() -> None:
    op.execute(
    ...
    try:
        op.execute(
            ...
            BEGIN
                ...
                WITH candidates AS (
                    ...
                    ORDER BY
                        c.process_after,c.materialized_at,c.command_id
                ...
                claimed AS (
                    ...
                    SET
                        status='processing',
                        attempt_count=CASE
                            WHEN x.reclaiming THEN c.attempt_count
                    ...
                    RETURNING
                        c.command_id,c.refund_id,c.payment_id,
                        c.organization_id,c.amount,c.currency_code,
                        c.attempt_count,c.lease_fence,c.leased_until,
                        x.reclaiming
                )
                SELECT
                    x.command_id,x.refund_id,x.payment_id,
                    x.organization_id,p.provider_code::text,
                    p.provider_payment_ref::text,x.amount,
                    x.currency_code,x.attempt_count,x.lease_fence,
                    x.reclaiming,x.leased_until
                FROM claimed x
                JOIN finance.payments p ON p.id=x.payment_id
        ...
        op.execute(
            ...
            AS $function$
            DECLARE
                v_c finance.refund_execution_commands%ROWTYPE;
                v_r finance.refunds%ROWTYPE;
                v_p finance.payments%ROWTYPE;
                v_allocated numeric;
                v_reserved numeric;
            BEGIN
                IF NOT pg_catalog.pg_has_role(
    ...
    _require_c_predecessor(bind)
    _install()
    _install_c_capabilities()
    _post_install_proof(bind)
    _post_install_c_proof(bind)
...
def downgrade() -> None:
    ...
    op.execute(
        ...
        """
                ...
                FOR UPDATE;
                IF NOT FOUND
                   OR v_c.status<>'processing'
                   OR v_c.leased_by IS DISTINCT FROM p_worker_id
                   OR v_c.lease_fence IS DISTINCT FROM p_lease_fence
                   OR v_c.leased_until<=pg_catalog.clock_timestamp()
                THEN
                    RAISE EXCEPTION
                ...
                IF v_r.payment_id<>v_p.id
                   OR v_c.payment_id<>v_p.id
                   OR v_c.refund_id<>v_r.id
                   OR v_c.organization_id
                        IS DISTINCT FROM v_p.organization_id
                   OR v_r.organization_id
                        IS DISTINCT FROM v_p.organization_id
                   OR v_c.legal_entity_id<>v_p.legal_entity_id
                   OR v_r.legal_entity_id<>v_p.legal_entity_id
                   OR v_c.division_id IS DISTINCT FROM v_p.division_id
                   OR v_r.division_id IS DISTINCT FROM v_p.division_id
                   OR v_c.brand_id IS DISTINCT FROM v_p.brand_id
                   OR v_r.brand_id IS DISTINCT FROM v_p.brand_id
                   OR v_c.amount<>v_r.amount
                   OR v_c.currency_code<>v_p.currency_code
                   OR v_r.currency_code<>v_p.currency_code
                THEN
                    RAISE EXCEPTION
                ...
                IF v_r.status NOT IN (
                    'requested','approved','processing'
                )
                   OR v_p.status NOT IN (
                       'captured','settled','partially_refunded'
                   )
                   OR v_p.organization_id IS NULL
                   OR nullif(btrim(v_p.provider_payment_ref),'') IS NULL
                   OR nullif(btrim(v_p.provider_code),'') IS NULL
                THEN
                    RAISE EXCEPTION
                ...
                IF p_provider_code IS DISTINCT FROM v_p.provider_code
                   OR p_provider_payment_ref
                        IS DISTINCT FROM v_p.provider_payment_ref
                   OR p_amount IS DISTINCT FROM v_c.amount
                   OR upper(btrim(coalesce(p_currency_code,'')))
                        IS DISTINCT FROM v_c.currency_code
                THEN
                    RAISE EXCEPTION
                ...
                IF v_c.provider_refund_ref IS NOT NULL THEN
                    RAISE EXCEPTION
                        'PAY-10 known provider refund must reconcile'
                        USING ERRCODE='23514';
                END IF;
                IF v_c.provider_code IS NOT NULL
                   AND v_c.provider_code<>v_p.provider_code
                THEN
                    RAISE EXCEPTION
                        'PAY-10 provider binding drift'
                        USING ERRCODE='23514';
                END IF;
                IF v_c.request_sha256 IS NOT NULL
                   AND v_c.request_sha256<>p_request_sha256
                THEN
                    RAISE EXCEPTION
                ...
                SET
                    ...
                    provider_code=coalesce(
                        c.provider_code,v_p.provider_code
                    ),
                    updated_at=pg_catalog.clock_timestamp()
                WHERE c.command_id=v_c.command_id
                ...
                RETURN QUERY SELECT
                    v_c.command_id,v_c.refund_id,v_c.payment_id,
                    v_c.organization_id,v_p.provider_code::text,
                    v_p.provider_payment_ref::text,v_c.amount,
                    v_c.currency_code,v_c.request_sha256::text,
                    v_c.status::text;
            END
            $function$
            """
        ...
        op.execute(
            ...
            DECLARE
                v_c finance.refund_execution_commands%ROWTYPE;
                v_e finance.refund_provider_evidence%ROWTYPE;
...
def downgrade() -> None:
    ...
    op.execute(
        ...
        """
                   ...
                   OR (
                       p_provider_refund_ref IS NOT NULL
                       AND (
...
def downgrade() -> None:
    ...
    op.execute(
        ...
        """
                ...
                THEN
                    RAISE EXCEPTION
                        'PAY-10 provider outcome shape invalid'
                        USING ERRCODE='22023';
                END IF;
                ...
                IF FOUND THEN
                    ...
                    END IF;
                    RETURN QUERY SELECT
                        v_existing.id,v_existing.command_id,
                        v_existing.normalized_status::text,
                        v_c.status::text,true;
                    RETURN;
                END IF;
                ...
                IF NOT FOUND
                   ...
                   OR v_c.lease_fence IS DISTINCT FROM p_lease_fence
                   OR v_c.leased_until<=pg_catalog.clock_timestamp()
                   OR v_c.request_sha256 IS NULL
                   OR v_c.provider_code IS NULL
                THEN
                    RAISE EXCEPTION
                        'PAY-10 stale or unbound refund execution'
                        USING ERRCODE='40001';
                END IF;
                IF v_c.provider_refund_ref IS NOT NULL
                   AND v_c.provider_refund_ref
                        IS DISTINCT FROM p_provider_refund_ref
                THEN
                    RAISE EXCEPTION
                ...
                INSERT INTO finance.refund_provider_evidence(
                    ...
                    provider_code,provider_event_id,provider_refund_ref,
                    evidence_source,normalized_status,request_sha256,
                    evidence_sha256,occurred_at
                )
                VALUES(
                    v_c.command_id,v_c.refund_id,v_c.payment_id,
                    v_c.organization_id,v_c.provider_code,NULL,
                    p_provider_refund_ref,'submission',
                    p_normalized_status,v_c.request_sha256,
                ...
                RETURN QUERY SELECT
                    v_e.id,v_e.command_id,
                    v_e.normalized_status::text,v_next,false;
            END
            $function$
            """
        )
        ...
        """
                ...
                v_next:=CASE
                    WHEN coalesce(p_permanent,false)
                         OR v_c.attempt_count>=v_c.max_attempts
                    THEN 'dead_lettered'
                    ELSE 'retry_pending'
                END;
                    ...
                    process_after=CASE
                        ...
                        THEN pg_catalog.clock_timestamp()+(
                            least(
                                1800,
                                30 * (
                                    2 ^ greatest(v_c.attempt_count-1,0)
                                )
                            ) * interval '1 second'
                        )
        ...
        op.execute(
            ...
            BEGIN
                IF NOT pg_catalog.pg_has_role(
                    ...
                    RAISE EXCEPTION
                        'PAY-10 evidence requires reconciliation runtime'
                        USING ERRCODE='42501';
                END IF;
...
def downgrade() -> None:
    ...
    op.execute(
        ...
        """
                   ...
                   OR (
                       p_evidence_source='webhook'
                       AND nullif(btrim(p_provider_event_id),'') IS NULL
                   )
                   OR (
...
def downgrade() -> None:
    ...
    op.execute(
        ...
        """
                ...
                THEN
                    RAISE EXCEPTION
                        'PAY-10 external evidence command invalid'
                        USING ERRCODE='23514';
                END IF;
                SELECT p.* INTO v_p
                FROM finance.payments p
                WHERE p.id=v_c.payment_id
                FOR UPDATE;
                IF NOT FOUND
                   OR v_p.provider_code<>v_c.provider_code
                   OR v_p.provider_payment_ref
                        IS DISTINCT FROM p_provider_payment_ref
                   OR v_p.organization_id
                        IS DISTINCT FROM v_c.organization_id
                THEN
                    RAISE EXCEPTION
                        'PAY-10 external evidence authority mismatch'
                        USING ERRCODE='23514';
                END IF;
                IF v_c.status='processing'
                   AND v_c.leased_until>pg_catalog.clock_timestamp()
                THEN
                    RAISE EXCEPTION
                ...
                IF v_c.provider_refund_ref IS NOT NULL
                   ...
                   AND v_c.provider_refund_ref<>p_provider_refund_ref
                THEN
                    RAISE EXCEPTION
                        'PAY-10 provider refund identity drift'
                        USING ERRCODE='23514';
                ...
                IF p_provider_event_id IS NOT NULL THEN
                    SELECT e.* INTO v_existing
                    FROM finance.refund_provider_evidence e
                    WHERE e.provider_code=v_c.provider_code
                      AND e.provider_event_id=p_provider_event_id;
                    IF FOUND THEN
                        IF v_existing.command_id<>v_c.command_id
                           OR v_existing.provider_refund_ref
                                IS DISTINCT FROM p_provider_refund_ref
                ...
                SELECT e.* INTO v_existing
                FROM finance.refund_provider_evidence e
                WHERE e.command_id=v_c.command_id
                  AND e.evidence_sha256=p_evidence_sha256;
                IF FOUND THEN
                ...
                VALUES(
                    v_c.command_id,v_c.refund_id,v_c.payment_id,
                    v_c.organization_id,v_c.provider_code,
                    p_provider_event_id,p_provider_refund_ref,
                    p_evidence_source,p_normalized_status,
                    v_c.request_sha256,p_evidence_sha256,p_occurred_at
                )
                RETURNING * INTO v_e;
                ...
                v_next:=CASE
                    WHEN v_c.status='succeeded' THEN 'succeeded'
                    WHEN v_processed THEN 'reconciliation_pending'
                    WHEN p_normalized_status='failed' THEN 'rejected'
                    WHEN v_c.status='reconciliation_pending'
                    THEN 'reconciliation_pending'
                ...
                SET
                    ...
                    END,
                    last_error_code=CASE
                        WHEN v_next='rejected' THEN 'provider_failed'
                        WHEN v_next='succeeded' THEN c.last_error_code
                        ELSE NULL
                    END,
                    updated_at=pg_catalog.clock_timestamp()
                WHERE c.command_id=v_c.command_id;
        ...
        for signature in (
            _CLAIM_REFUND,_BIND_REQUEST,_RECORD_OUTCOME,
            _RECORD_UNKNOWN,_RECORD_FAILURE,
        ):
            op.execute(
...
def _post_install_c_proof(bind) -> None:
    if bind.execute(
        sa.text(
            "SELECT pg_catalog.has_schema_privilege("
            "'app_security_owner','app_secure','CREATE')"
        ...
        raise RuntimeError("PAY-10-C temporary CREATE authority leaked")
    ...
    runtime_roles = (
        "app_runtime","worker_runtime","finance_payment_runtime",
        _REFUND_RUNTIME,_RECON_RUNTIME,"finance_maintenance_runtime",
    )
    for signature, allowed in expected.items():
        ...
        if (
            row is None
            or row[0] != _SECURITY_OWNER
            or not bool(row[1])
            or "row_security=on" not in (row[2] or "")
        ):
            raise RuntimeError(
        ...
        for role in runtime_roles:
            ...
            if bool(actual) != (role in allowed):
                raise RuntimeError(
                    f"PAY-10-C execute ACL drift: {role} -> {signature}"
                )
...
def _drop_c_capabilities() -> None:
    ...
    try:
        ...
        for signature in reversed(_PAY10_C_FUNCTIONS):
