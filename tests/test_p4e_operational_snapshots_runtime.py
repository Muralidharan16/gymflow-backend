...
def test_maintenance_task_reads_exact_certified_aggregate_snapshots(
    ...
    for key in _SEARCH_COLUMNS[:-1]:
        assert actual["search"][key] == before_search[key] == after_search[key]
    for key in _REFUND_COLUMNS[:-1]:
        assert actual["refund"][key] == before_refund[key] == after_refund[key]
    assert (
        before_search["oldest_actionable_age_seconds"]
        <= actual["search"]["oldest_actionable_age_seconds"]
        <= after_search["oldest_actionable_age_seconds"]
    )
    assert (
        before_refund["oldest_unresolved_age_seconds"]
        <= actual["refund"]["oldest_unresolved_age_seconds"]
        <= after_refund["oldest_unresolved_age_seconds"]
    )
