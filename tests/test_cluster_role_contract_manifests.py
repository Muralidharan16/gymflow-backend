...
def test_all_five_machine_readable_manifests_exist() -> None:
    ...
    # Source-evidence portability contract.
    def source_evidence_path_is_portable(
        value: object,
    ) -> bool:
    ...
    rejected_source_paths = (
        "/" + "home/"
        + "jeeva" + "shri/evidence/artifact.json",
        "/" + "Users/"
        + "jeeva" + "shri/evidence/artifact.json",
        "C:" + chr(92) + "Users" + chr(92)
        + "jeeva" + "shri"
        + chr(92) + "artifact.json",
    ...
    for rejected_path in rejected_source_paths:
        assert not source_evidence_path_is_portable(
            rejected_path
        )
    ...
    for filename in MANIFEST_FILES.values():
        ...
        for record in source_evidence.values():
            ...
            assert source_evidence_path_is_portable(
                evidence_path
            )
            assert isinstance(
                evidence_sha256,
...
def test_exact_managed_role_contract() -> None:
    ...
    assert (
        roles["migration_owner"]["attributes"][
            "create_role"
        ]
        is False
    )
...
def test_ownership_manifest_uses_only_allowed_owners() -> None:
    ...
    for record in ownership["objects"]:
        assert (
            record["target_owner"]
            in ownership["allowed_target_owners"]
        )
...
def test_cluster_roles_and_memberships_survive_downgrade() -> None:
    ...
    assert (
        bundle.roles["downgrade_policy"][
            "cluster_roles_survive_database_downgrade"
        ]
        is True
    )

    assert (
        bundle.memberships["downgrade_policy"][
            "bootstrap_memberships_survive_database_downgrade"
        ]
        is True
    )

    assert (
        bundle.role_settings["downgrade_policy"][
            "role_settings_survive_database_downgrade"
        ]
        is True
    )
