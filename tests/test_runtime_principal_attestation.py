...
def _canonical_observation(component: str, username: str, database: str = "gymflow_runtime_test") -> RuntimePrincipalObservation:
...
def _codes(observation: RuntimePrincipalObservation) -> set[str]:
    return {item.code for item in evaluate_runtime_principal_observation(observation)}
...
def test_runtime_binding_contract_matches_p2b_p2c_role_model() -> None:
    ...
    assert set(contract.bindings["api"].direct_capabilities) == {"app_runtime", "app_user"}
    assert set(contract.bindings["auth"].direct_capabilities) == {"auth_runtime", "app_runtime", "app_user"}
    assert contract.bindings["worker"].direct_capabilities == ("worker_runtime",)
    assert contract.bindings["maintenance"].direct_capabilities == ("lifecycle_maintenance_runtime",)
...
def test_same_login_behind_different_urls_is_rejected_before_connect() -> None:
    ...
    assert "runtime.config.login_reuse" in {item.code for item in validate_runtime_url_configuration(urls)}
...
def test_runtime_urls_must_target_same_database() -> None:
    ...
    assert "runtime.config.database_divergence" in {item.code for item in validate_runtime_url_configuration(urls)}


def test_current_user_contamination_is_rejected() -> None:
    observation = replace(_canonical_observation("worker", "worker_login"), current_user="app_runtime")
    assert "runtime.current_user_mismatch" in _codes(observation)


def test_bypassrls_on_deployment_login_is_rejected() -> None:
    observation = replace(_canonical_observation("worker", "worker_login"), bypass_rls=True)
...
def test_extra_direct_capability_is_rejected() -> None:
    ...
    drifted = replace(observation, memberships=(*observation.memberships, extra))
...
def test_wrong_grantor_is_rejected() -> None:
    ...
    assert "runtime.membership_grantor" in _codes(replace(observation, memberships=(row,)))
...
def test_set_role_option_on_runtime_capability_is_rejected() -> None:
    ...
    next(item for item in checks if item["target"] == "worker_runtime" and item["semantic"] == "SET")["allowed"] = True
    drifted = replace(observation, memberships=(row,), semantic_checks=tuple(checks))
...
def test_transitive_peer_reachability_is_rejected_by_live_semantics() -> None:
    observation = _canonical_observation("worker", "worker_login")
    checks = [dict(item) for item in observation.semantic_checks]
    next(item for item in checks if item["target"] == "app_runtime" and item["semantic"] == "MEMBER")["allowed"] = True
    assert "runtime.semantic_reachability" in _codes(replace(observation, semantic_checks=tuple(checks)))
...
def test_runtime_binding_set_rejects_login_reuse() -> None:
    ...
    assert "runtime.binding_set.login_reuse" in {item.code for item in evaluate_runtime_binding_set(observations)}


def test_repository_runtime_identity_routing_guard_passes() -> None:
    result = subprocess.run(
        [sys.executable, "-s", str(ROOT / "scripts/verify_runtime_identity_routing.py")],
        cwd=ROOT,
        check=False,
        capture_output=True,
        text=True,
