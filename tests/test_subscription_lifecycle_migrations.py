...
def new_seed() -> LifecycleSeed:
    ...
    values = [str(uuid.uuid4()) for _ in range(17)]
...
async def seed_v2_source_data(seed: LifecycleSeed) -> None:
    ...
    params = {
        "org1": seed.org_1, "org2": seed.org_2,
        "owner1": seed.owner_1, "owner2": seed.owner_2,
        "branch1": seed.branch_1, "branch2": seed.branch_2, "branch3": seed.branch_3,
        "member100": seed.member_100, "member101": seed.member_101, "member200": seed.member_200,
        "planIndividual": seed.plan_individual, "planFamily": seed.plan_family, "planOther": seed.plan_other,
        "subA": seed.sub_a, "subB": seed.sub_b, "subFamily": seed.sub_family, "subExpired": seed.sub_expired,
        "suffix": seed.suffix,
        "memberNo100": member_base, "memberNo101": member_base + 1, "memberNo200": member_base + 2,
        "phone100": f"9{phone_base:09d}", "phone101": f"8{phone_base:09d}", "phone200": f"7{phone_base:09d}",
    }
    statements = [
        """
...
async def test_lifecycle_migration_round_trip_preserves_v2_and_backfills_conservatively():
    ...
    p = _subscription_params(seed)
    id_filter = "(:sub_a, :sub_b, :sub_family, :sub_expired)"

    assert await scalar_int(f"SELECT count(*) FROM member_subscriptions_v2 WHERE id IN {id_filter}", p) == 4
    assert await scalar_int(f"SELECT count(*) FROM subscription_series WHERE legacy_member_subscription_v2_id IN {id_filter}", p) == 4
    assert await scalar_int(f"SELECT count(*) FROM subscription_terms WHERE legacy_member_subscription_v2_id IN {id_filter}", p) == 4
    assert await scalar_int(
        f"SELECT count(*) FROM subscription_term_slots s JOIN subscription_terms t ON t.id=s.term_id WHERE t.legacy_member_subscription_v2_id IN {id_filter}", p
    ) == 6
    assert await scalar_int(
        f"SELECT count(*) FROM subscription_slot_assignments a JOIN subscription_terms t ON t.id=a.term_id WHERE t.legacy_member_subscription_v2_id IN {id_filter}", p
    ) == 5
    assert await scalar_int(
        f"SELECT count(*) FROM subscription_terms WHERE legacy_member_subscription_v2_id IN {id_filter} AND renewed_from_term_id IS NOT NULL", p
    ) == 0
    assert await scalar_int(
        "SELECT count(DISTINCT series_id) FROM subscription_terms WHERE legacy_member_subscription_v2_id IN (:sub_a,:sub_b)", p
    ) == 2
    ...
    """, {"sub_family": seed.sub_family}) == 3
    assert await scalar_int("""
        SELECT count(*) FROM subscription_slot_assignments a JOIN subscription_terms t ON t.id=a.term_id
        WHERE t.legacy_member_subscription_v2_id=:sub_family
    """, {"sub_family": seed.sub_family}) == 2

    run_alembic("downgrade", BASELINE_REVISION)
    await assert_table_absent("subscription_series")
    assert await scalar_int(f"SELECT count(*) FROM member_subscriptions_v2 WHERE id IN {id_filter}", p) == 4
    assert await scalar_int(f"SELECT count(*) FROM subscription_members WHERE subscription_id IN {id_filter}", p) == 5
...
async def test_lifecycle_constraints_enforce_overlap_tenant_lineage_and_assignment_integrity():
    ...
    async with MigrationTestSessionLocal() as session:
        ...
        other_series = (
            await session.execute(text("SELECT series_id FROM subscription_terms WHERE legacy_member_subscription_v2_id=:sub_family"), {"sub_family": seed.sub_family})
        ).scalar_one()
        slot_id = (
            await session.execute(text("SELECT id FROM subscription_term_slots WHERE term_id=:term_id AND slot_index=1"), {"term_id": term["id"]})
        ).scalar_one()

    base = {
        "org_id": term["org_id"], "branch_id": term["branch_id"], "series_id": term["series_id"],
        "plan_id": term["plan_id"], "parent_term_id": term["id"], "other_series_id": other_series,
        "slot_id": slot_id, "member100": seed.member_100, "owner1": seed.owner_1,
    }
    ...
    """
    await expect_db_error(term_prefix + """
    ...
    """
    await expect_db_error(assignment_prefix + """
    ...
    adjacent = {**base, "adjacent_code": adjacent_code}
    async with MigrationTestSessionLocal() as session:
        await session.execute(text("""
            INSERT INTO subscription_terms (
              id, org_id, branch_id, series_id, sequence_number, term_code, renewed_from_term_id, source_type,
    ...
    assert await scalar_int("SELECT count(*) FROM subscription_terms WHERE term_code=:code", {"code": adjacent_code}) == 1
    async with MigrationTestSessionLocal() as session:
        await session.execute(text("DELETE FROM subscription_terms WHERE term_code=:code"), {"code": adjacent_code})
        await session.commit()
