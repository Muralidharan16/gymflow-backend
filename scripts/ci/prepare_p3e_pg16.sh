...
if [[ -n "${CERTIFICATION_HEAD:-}" ]]; then
  test "$(python -s -m alembic -c alembic.ini heads | awk '{print $1}')" = "${CERTIFICATION_HEAD}"
else
  test "$(python -s -m alembic -c alembic.ini heads | awk '{print $1}')" = 't07d8e9f0a34'
fi
