#!/usr/bin/env python3
"""Bind PR-side P10-S to the canonical same-head push soak artifact."""

from __future__ import annotations

import argparse
import hashlib
import json
import os
import shutil
import time
import urllib.request
import zipfile
from io import BytesIO
from pathlib import Path, PurePosixPath

from github_artifact_transport import download_github_artifact

WORKFLOW_PATH = ".github/workflows/p10s-long-soak.yml"
SOURCE_EVENT = "push"
REQUIRED_FILES = (
    "http.json",
    "worker.json",
    "resources.jsonl",
    "decision.json",
    "decision.log",
    "runtime-metrics.jsonl",
)


def _get_json(url: str, token: str) -> dict:
    request = urllib.request.Request(
        url,
        headers={
            "Accept": "application/vnd.github+json",
            "Authorization": f"Bearer {token}",
            "X-GitHub-Api-Version": "2022-11-28",
        },
    )
    with urllib.request.urlopen(request, timeout=60) as response:
        return json.load(response)


def _safe_extract(raw: bytes, target: Path) -> None:
    with zipfile.ZipFile(BytesIO(raw)) as archive:
        members = archive.infolist()
        if not members:
            raise RuntimeError("P10-S evidence archive is empty")
        for member in members:
            path = PurePosixPath(member.filename)
            if path.is_absolute() or ".." in path.parts:
                raise RuntimeError(f"unsafe artifact path: {member.filename!r}")
        archive.extractall(target, members=members)


def _locate(root: Path, name: str) -> Path:
    matches = [path for path in root.rglob(name) if path.is_file()]
    if len(matches) != 1:
        raise RuntimeError(f"expected exactly one {name}, found {len(matches)}")
    return matches[0]


def main() -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("--repo", required=True)
    parser.add_argument("--sha", required=True)
    parser.add_argument("--output-dir", required=True)
    parser.add_argument("--max-wait-seconds", type=int, default=1800)
    args = parser.parse_args()

    token = os.environ.get("GH_TOKEN", "")
    if not token:
        raise SystemExit("GH_TOKEN is required")
    if len(args.sha) != 40:
        raise SystemExit("exact 40-character candidate SHA is required")

    api = os.environ.get("GITHUB_API_URL", "https://api.github.com")
    deadline = time.monotonic() + args.max_wait_seconds
    selected: dict | None = None
    last_state = "not found"

    while time.monotonic() < deadline:
        payload = _get_json(
            f"{api}/repos/{args.repo}/actions/runs"
            f"?head_sha={args.sha}&event={SOURCE_EVENT}&per_page=100",
            token,
        )
        candidates = [
            run
            for run in payload.get("workflow_runs", [])
            if run.get("path") == WORKFLOW_PATH
            and run.get("head_sha") == args.sha
            and run.get("event") == SOURCE_EVENT
        ]
        if candidates:
            selected = max(candidates, key=lambda item: int(item["id"]))
            last_state = (
                f"id={selected['id']} status={selected['status']} "
                f"conclusion={selected.get('conclusion')}"
            )
            if selected["status"] == "completed":
                break
        time.sleep(10)

    if selected is None or selected["status"] != "completed":
        raise RuntimeError(f"same-head canonical P10-S run did not complete: {last_state}")
    if selected.get("conclusion") != "success":
        raise RuntimeError(f"same-head canonical P10-S run is not successful: {last_state}")

    artifact_payload = _get_json(selected["artifacts_url"] + "?per_page=100", token)
    expected_name = f"p10s-{args.sha}"
    artifacts = [
        item
        for item in artifact_payload.get("artifacts", [])
        if item.get("name") == expected_name and not item.get("expired")
    ]
    if len(artifacts) != 1:
        raise RuntimeError(
            f"expected one retained canonical artifact {expected_name!r}, "
            f"found {len(artifacts)}"
        )

    artifact = artifacts[0]
    archive_bytes = download_github_artifact(artifact["archive_download_url"], token)
    output = Path(args.output_dir)
    output.mkdir(parents=True, exist_ok=True)
    extracted = output / "_canonical_push_soak"
    if extracted.exists():
        shutil.rmtree(extracted)
    extracted.mkdir(parents=True)
    _safe_extract(archive_bytes, extracted)

    file_digests: dict[str, str] = {}
    for name in REQUIRED_FILES:
        source = _locate(extracted, name)
        target = output / name
        shutil.copy2(source, target)
        file_digests[name] = hashlib.sha256(target.read_bytes()).hexdigest()

    decision = json.loads((output / "decision.json").read_text(encoding="utf-8"))
    if decision.get("phase") != "P10-S" or decision.get("decision") != "PASS":
        raise RuntimeError("canonical P10-S decision is not PASS")
    if int(decision.get("duration_seconds", 0)) < 300:
        raise RuntimeError("canonical P10-S duration is below 300 seconds")
    if int(decision.get("windows", 0)) < 5:
        raise RuntimeError("canonical P10-S has fewer than five windows")
    if decision.get("provider_execution") != "DEFERRED_FAIL_CLOSED":
        raise RuntimeError("canonical P10-S provider boundary drifted")

    provenance = {
        "schema_version": 1,
        "phase": "P10-S",
        "candidate_sha": args.sha,
        "canonical_event": SOURCE_EVENT,
        "source_workflow": WORKFLOW_PATH,
        "source_run_id": selected["id"],
        "source_run_conclusion": selected["conclusion"],
        "artifact_id": artifact["id"],
        "artifact_name": artifact["name"],
        "artifact_archive_sha256": hashlib.sha256(archive_bytes).hexdigest(),
        "github_artifact_digest": artifact.get("digest"),
        "bound_file_sha256": file_digests,
        "decision": "BOUND",
    }
    (output / "p10s-provenance.json").write_text(
        json.dumps(provenance, indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    print(json.dumps(provenance, indent=2, sort_keys=True))
    print("P10S_SAME_HEAD_SOAK_BOUND=PASS")
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
