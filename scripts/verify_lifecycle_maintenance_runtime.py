...
STAGES = (
    "login_posture", "acl_contract", "context_gate",
    "positive_maintenance", "negative_boundaries",
)


def _sync_url(env_name: str) -> str:
    return os.environ[env_name].replace("postgresql+asyncpg://", "postgresql+psycopg://")
...
def _set_context(conn: sa.Connection, *, org_id: uuid.UUID | None = None) -> None:
    ...
    if org_id is not None:
        conn.execute(sa.text(
            "SELECT pg_catalog.set_config('app.current_org_id', :value, true)"
        ), {"value": str(org_id)})
        conn.execute(sa.text(
...
def _assert_login_posture(engine: sa.Engine, expected_user: str) -> None:
    with engine.begin() as conn:
        ...
        assert not conn.execute(sa.text(
...
def stage_login_posture(engines: dict[str, sa.Engine]) -> None:
    for key, user in (
        ("api", "app_test_runtime"), ("auth", "auth_test_runtime"),
        ("worker", "worker_test_runtime"), ("maintenance", "maintenance_test_runtime"),
    ):
        _assert_login_posture(engines[key], user)


def stage_acl_contract(engines: dict[str, sa.Engine]) -> None:
    with engines["api"].begin() as conn:
        observed = _column_updates(conn, "app_runtime")
        assert observed == API_COLUMNS, (sorted(API_COLUMNS), sorted(observed))
        assert not conn.execute(sa.text(
            "SELECT pg_catalog.has_table_privilege('app_runtime','public.org_branch_state','UPDATE')"
        )).scalar_one()
        for privilege in ("SELECT", "INSERT"):
            assert not conn.execute(sa.text(
                "SELECT pg_catalog.has_table_privilege('app_runtime','public.branch_watchdog_alerts',:p)"
            ), {"p": privilege}).scalar_one()
    with engines["maintenance"].begin() as conn:
        observed = _column_updates(conn, "lifecycle_maintenance_runtime")
        assert observed == MAINTENANCE_COLUMNS, (sorted(MAINTENANCE_COLUMNS), sorted(observed))
        assert conn.execute(sa.text(
            "SELECT pg_catalog.has_table_privilege('lifecycle_maintenance_runtime','public.org_branch_state','SELECT')"
        )).scalar_one()
        assert not conn.execute(sa.text(
            "SELECT pg_catalog.has_table_privilege('lifecycle_maintenance_runtime','public.org_branch_state','UPDATE')"
        )).scalar_one()
        for privilege in ("SELECT", "INSERT"):
            assert conn.execute(sa.text(
                "SELECT pg_catalog.has_table_privilege('lifecycle_maintenance_runtime','public.branch_watchdog_alerts',:p)"
            ), {"p": privilege}).scalar_one()
...
def stage_context_gate(engines: dict[str, sa.Engine]) -> None:
    ...
    with maintenance.begin() as conn:
        ...
        ), {"branch_id": BRANCH_ID}).scalar_one() == 0
        assert conn.execute(sa.text(
            "UPDATE public.org_branch_state SET reconciliation_claimed_by=:claim_id WHERE branch_id=:branch_id"
...
def stage_positive_maintenance(engines: dict[str, sa.Engine]) -> None:
    with engines["maintenance"].begin() as conn:
        _set_context(conn)
        row = conn.execute(sa.text(
            "SELECT status,search_visibility_version FROM public.org_branch_state WHERE branch_id=:branch_id"
...
def stage_negative_boundaries(engines: dict[str, sa.Engine]) -> None:
    api, auth = engines["api"], engines["auth"]
    worker, maintenance = engines["worker"], engines["maintenance"]
    _expect_db_denial(maintenance,
        "UPDATE public.org_branch_state SET status='suspended' WHERE branch_id=:branch_id",
        {"branch_id": BRANCH_ID})
    _expect_db_denial(maintenance, "SELECT * FROM public.branch_outbox_events LIMIT 1")
    _expect_db_denial(maintenance,
        "INSERT INTO public.organizations (id,name,tier,is_active) VALUES (gen_random_uuid(),'forbidden','basic',true)")
    _expect_db_denial(api,
        "UPDATE public.org_branch_state SET reconciliation_claimed_at=clock_timestamp() WHERE branch_id=:branch_id",
        {"branch_id": BRANCH_ID})
    ...
    for engine, identity in ((worker, "worker"), (auth, "auth")):
        try:
            ...
            continue
        raise AssertionError(f"{identity} identity updated maintenance reconciliation state")
...
def main() -> None:
    ...
    for stage in selected:
        ...
        try:
            ...
            print(f"VERIFY_STAGE_FAILURE: {stage}: {type(exc).__name__}: {exc}", flush=True)
            raise
