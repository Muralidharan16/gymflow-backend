...
class PlatformDunningCase(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("status <> 'suspended' OR suspended_at IS NOT NULL", name="chk_platform_dunning_cases_suspended_shape"),
        CheckConstraint("status <> 'terminated' OR terminated_at IS NOT NULL", name="chk_platform_dunning_cases_terminated_shape"),
...
class PlatformNotificationDelivery(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "notification_type IN ('payment_failed','retry_scheduled','grace_ending','access_limited','access_read_only','subscription_suspended','payment_recovered','mandate_expiring','mandate_revoked','payment_method_replaced')",
