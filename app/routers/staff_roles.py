...
def _require_staff_roster_scope(branch_id: UUID, staff: Staff) -> None:
    """Fail closed for branch-scoped callers before enabling staff-role reads."""
...
async def create_org_user(
    ...
    async with managed_db_write(session, circuit_breaker=staff_write_circuit_breaker):
        await set_session_context(session, org_id=current_staff.org_id, user_id=current_staff.id)
...
async def assign_staff_role(
    ...
    async with managed_db_write(session, circuit_breaker=staff_write_circuit_breaker):
        ...
        # require_org_admin + tenant-bound branch validation precede the
        # transaction-local RLS read capability.
...
async def list_branch_staff(
    ...
    # The RLS read capability is intentionally not installed until both tenant
    # ownership and caller branch scope have been proven.
    await validate_branch_ownership(branch_id, current_staff.org_id, session)
    _require_staff_roster_scope(branch_id, current_staff)
    await set_session_context(session, org_id=current_staff.org_id)
