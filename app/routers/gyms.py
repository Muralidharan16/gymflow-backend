...
@role_required([StaffRole.owner, StaffRole.admin])
...
@role_required([StaffRole.owner, StaffRole.admin])
async def update_tax_config(request: Request, gym_id: uuid.UUID, data: TaxConfigCreate, db: AsyncSession = Depends(get_db)):
    service = GymService(GymRepository(db), TaxRepository(db), db)
