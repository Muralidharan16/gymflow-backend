...
class StaffRolesService:
    ...
    async def authorize_staff_roles_read(self) -> None:
        ...
        await self.session.execute(
            text(
                ...
                "'app.can_read_staff_roles', 'true', true)"
            )
        )
    ...
    async def update_organization_user(self, user_id: UUID, org_id: UUID, data: OrganizationUserUpdate) -> OrganizationUser:
        user = await self.repo.get_user_by_id(user_id, org_id)
        if not user:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Organization user not found.")
    ...
    async def assign_branch_staff_role(self, branch_id: UUID, org_id: UUID, assigned_by: UUID, data: BranchStaffRoleCreate) -> BranchStaffRole:
        ...
        # UX fast path; PostgreSQL's exclusion constraint remains authoritative
        ...
        for assign in active_assignments:
            requested_from = data.effective_from.astimezone(timezone.utc)
            requested_to = (
                data.effective_to.astimezone(timezone.utc)
                if data.effective_to
                else datetime.max.replace(tzinfo=timezone.utc)
            )
            existing_from = assign.effective_from.astimezone(timezone.utc)
            existing_to = (
                assign.effective_to.astimezone(timezone.utc)
                if assign.effective_to
                else datetime.max.replace(tzinfo=timezone.utc)
            )
            if requested_from < existing_to and existing_from < requested_to:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
    ...
    async def revoke_branch_staff_role(self, branch_id: UUID, assignment_id: UUID, org_id: UUID, revoked_by: UUID) -> BranchStaffRole:
        ...
        if not assignment or assignment.branch_id != branch_id:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Branch staff role assignment not found.")
        if assignment.revoked_at is not None:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="This role assignment has already been revoked.")
    ...
    async def list_branch_staff(self, branch_id: UUID, org_id: UUID, include_inactive: bool = False) -> List[BranchStaffRole]:
