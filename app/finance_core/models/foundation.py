...
",
        ...
        CheckConstraint(
            "provider_object_id IS NULL OR provider_object_id ~ '^[A-Za-z0-9_:-]{1,200}    __tablename__ = "payments"
    __table_args__ = (
        UniqueConstraint("provider_code", "provider_payment_ref", name="uq_finance_payments_provider_payment_ref"),
        UniqueConstraint("id", "currency_code", name="uq_finance_payments_id_currency"),
        CheckConstraint("provider_code ~ '^[a-z0-9_]+$'", name="chk_finance_payments_provider_code"),
...
",
        ...
        CheckConstraint(
            "provider_evidence_sha256 IS NULL OR provider_evidence_sha256 ~ '^[0-9a-f]{64}    __tablename__ = "payments"
    __table_args__ = (
        UniqueConstraint("provider_code", "provider_payment_ref", name="uq_finance_payments_provider_payment_ref"),
        UniqueConstraint("id", "currency_code", name="uq_finance_payments_id_currency"),
        CheckConstraint("provider_code ~ '^[a-z0-9_]+$'", name="chk_finance_payments_provider_code"),
...
class FinanceOutboxEvent(Base):
 ...
 "
        ...
        CheckConstraint(
            ...
            "AND provider_evidence_sha256 IS NOT NULL AND completed_at IS NOT NULL) "
            "OR (status='failed_final' AND completed_at IS NOT NULL) "
            "OR (status NOT IN ('succeeded','failed_final') AND completed_at IS NULL)",
