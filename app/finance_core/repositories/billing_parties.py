...
class FinanceBillingPartyRepository:
    ...
    async def get_organization(self, organization_id: uuid.UUID, *, for_update: bool = False) -> Organization | None:
    ...
    async def acquire_organization_creation_lock(self, organization_id: uuid.UUID) -> None:
    ...
    async def get_by_organization(
        ...
        *,
        ...
        statement = select(FinanceBillingParty).where(FinanceBillingParty.organization_id == organization_id)
        if for_update:
            statement = statement.with_for_update()
        result = await self._session.execute(statement)
        return result.scalar_one_or_none()

    async def get_by_id(self, billing_party_id: uuid.UUID, *, for_update: bool = False) -> FinanceBillingParty | None:
        statement = select(FinanceBillingParty).where(FinanceBillingParty.id == billing_party_id)
        if for_update:
            statement = statement.with_for_update()
        result = await self._session.execute(statement)
        return result.scalar_one_or_none()
