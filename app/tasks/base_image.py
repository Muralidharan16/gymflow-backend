...
def run_image_pipeline(org_id: str, upload_id: str, user_id: str, request_ip: Optional[str], config: AssetPipelineConfig) -> bool:
    ...
    def mark_db_failed(reason: str, extra: Optional[dict] = None):
        ...
        with WorkerSyncSessionLocal() as db:
            ...
            if org:
                ...
                audit = OrganizationAssetAudit(org_id=org_id, changed_by=user_id, asset_type=config.asset_type, action="failed", action_detail=detail, ip_address=request_ip)
                db.add(audit)
                db.commit()
    ...
    try:
        ...
        for size_name, target_width in config.sizes.items():
            processed_img = resize_and_pad(img_clean, target_width, target_width) if config.resize_strategy == "pad" else resize_cover_focal(img_clean, target_width, config.focal_y)
            ...
            s3.put_object(Bucket=settings.S3_BUCKET_NAME, Key=s3_key, Body=buffer.getvalue(), ContentType='image/webp')
            keys[size_name] = s3_key
        original_key = f"originals/{org_id}/{asset_uuid}_original"
        s3.put_object(Bucket=settings.S3_BUCKET_NAME, Key=original_key, Body=file_bytes, ContentType='image/jpeg' if file_bytes[:3] == b'\xff\xd8\xff' else ('image/webp' if file_bytes[8:12] == b'WEBP' else 'image/png'))
    ...
    with WorkerSyncSessionLocal() as db:
        ...
        if org:
            if config.asset_type == "logo":
                ...
                org.logo_meta = {"width": w, "height": h, "size_bytes": len(file_bytes), "mime_type": "image/webp"}
                ...
                org.cover_meta = {"width": w, "height": h, "size_bytes": len(file_bytes), "mime_type": "image/webp", "focal_point_y": config.focal_y}
                org.cover_updated_at = datetime.utcnow()
                org.cover_updated_by = user_id
            audit = OrganizationAssetAudit(org_id=org_id, changed_by=user_id, asset_type=config.asset_type, action="uploaded", old_s3_key=old_keys[0] if old_keys else None, new_s3_key=keys["original"], ip_address=request_ip)
            db.add(audit)
            db.commit()
    s3.delete_object(Bucket=settings.S3_BUCKET_NAME, Key=quarantine_key)
    if old_keys:
