...
async def verify_audit_partition_readiness() -> None:
    ...
    Partition creation is an infrastructure concern owned by pg_partman.  The
    application runtime intentionally has no CREATE privilege on the database
    or public schema.  This monitor therefore reads PostgreSQL catalogs only and
    fails if the managed partition set has fallen behind.
    """
...
def run() -> None:
    """Read-only operational monitor for externally maintained partitions."""
