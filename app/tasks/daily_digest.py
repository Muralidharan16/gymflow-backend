...
async def daily_digest_task():
    async with AsyncSessionLocal() as session:
        ...
        for gym in gyms:
            ...
            collections = (await session.execute(q_pay)).scalar() or 0
            
            # 2. Expired
            q_exp = select(func.count(MemberSubscription.id)).where(
                MemberSubscription.gym_id == gym.id,
                MemberSubscription.end_date == yesterday
            )
            expired_count = (await session.execute(q_exp)).scalar() or 0
            
            # 3. Expiring soon
            q_soon = select(func.count(MemberSubscription.id)).where(
                MemberSubscription.gym_id == gym.id,
                MemberSubscription.end_date.in_([date.today(), date.today() + timedelta(days=1)])
            )
            expiring_soon = (await session.execute(q_soon)).scalar() or 0
            
            # Generate PDF
            pdf_bytes = generate_digest_pdf(gym.name, str(collections), expired_count, expiring_soon)
            ...
            if owner and owner.phone:
                ...
                await send_whatsapp_document(owner.phone, pdf_url, f"Daily_Report_{gym.name}_{date.today()}.pdf")
                gyms_processed += 1
