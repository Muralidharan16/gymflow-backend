...
def process_org_logo(self, org_id: str, upload_id: str, user_id: str, request_ip: str = None):
    config = AssetPipelineConfig(
        asset_type="logo",
        sizes={"thumb": 64, "medium": 256, "full": 1024},
        resize_strategy="pad",
        allow_svg=False,  # Hard-banned for now to prevent transcode silent failures and XSS
...
def cleanup_orphaned_logos():
    """Finds files in S3 under logos/, covers/, and originals/ prefixes that are older than 24h and not in the DB, and deletes them."""
    s3 = get_s3_client()
    import datetime
    ...
    # Efficient pagination of DB keys to avoid OOM
    valid_keys = set()
    offset = 0
    batch_size = 500
        ...
        while True:
            orgs = (
                ...
                .with_entities(
                    ...
                    Organization.logo_thumb_key,
                    Organization.logo_medium_key,
                    Organization.logo_full_key,
                    Organization.cover_key,
                    Organization.cover_mobile_key,
                    Organization.cover_tablet_key,
                    Organization.cover_desktop_key,
                )
                .offset(offset)
                .limit(batch_size)
                .all()
    ...
    # Scan logos/
    for page in paginator.paginate(Bucket=settings.S3_BUCKET_NAME, Prefix='logos/'):
    ...
    # Scan covers/
    for page in paginator.paginate(Bucket=settings.S3_BUCKET_NAME, Prefix='covers/'):
        if 'Contents' not in page:
            continue
        for obj in page['Contents']:
            if obj['LastModified'] < threshold_date:
    ...
    # Scan originals/
    for page in paginator.paginate(Bucket=settings.S3_BUCKET_NAME, Prefix='originals/'):
        if 'Contents' not in page:
            continue
        for obj in page['Contents']:
            if obj['LastModified'] < threshold_date:
