...
class BranchStatusTransition(Base):
    ...
    from_status: Mapped[str] = mapped_column(
        String, ForeignKey("branch_status_definitions.code"), primary_key=True
    )
...
class BranchDeactivationPolicy(Base):
    ...
    booking_grace_hours: Mapped[int] = mapped_column(
        Integer, default=24, server_default=text("24"), nullable=False
    )
    auto_cancel_bookings: Mapped[bool] = mapped_column(
        Boolean, default=False, server_default=text("FALSE"), nullable=False
    )
    notify_members: Mapped[bool] = mapped_column(
    ...
    __table_args__ = (
        sa.ForeignKeyConstraint(
            ...
            [
                "branch_status_transitions.from_status",
                "branch_status_transitions.to_status",
            ],
        ),
    )
...
class BranchStatusHistory(Base):
    ...
    branch_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("org_branches.id"), nullable=False
    )
    from_status: Mapped[Optional[str]] = mapped_column(
        String, ForeignKey("branch_status_definitions.code"), nullable=True
    )
    to_status: Mapped[str] = mapped_column(
        String, ForeignKey("branch_status_definitions.code"), nullable=False
    )
    ...
    correlation_emitted_at: Mapped[Optional[datetime]] = mapped_column(
...
class BranchLifecycleEvent(Base):
    ...
    emitted_at: Mapped[datetime] = mapped_column(
        TIMESTAMP(timezone=True), primary_key=True, server_default=text("clock_timestamp()")
    )
...
class BranchOutboxEvent(Base):
    ...
    __table_args__ = (
        sa.CheckConstraint(
            ...
            name="chk_branch_outbox_attempt_bounds",
        ),
        sa.CheckConstraint(
